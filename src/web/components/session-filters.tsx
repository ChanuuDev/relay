...
export function SessionFiltersForm({ children }: { children: ReactNode }) {
  ...
  return <form className="sessions-header" onSubmit={(event) => {
    ...
    {children}
    {advanced && <FieldGroup id="advanced-filters" className="advanced-filters material-chrome">
...
export function FilterChips({ params, activeFilters }: { params: URLSearchParams; activeFilters: string[] }) {
  ...
  return <div className="filter-chips material-chrome" role="group" aria-label="적용된 필터">
    {activeFilters.map((key) => <Badge variant="secondary" key={key}>{params.get(key)}
