...
test("install-hooks registers Claude, Grok and Codex SessionStart without clobbering other hooks", () => {
  ...
  if (process.platform === "win32") {
    expect(first.wrappers.map(w => w.status)).toEqual(["added", "added"]);
...
test("install-hooks updates a moved relay path and skips invalid JSON", () => {
  ...
  expect(JSON.parse(readFileSync(path.join(home(), ".claude", "settings.json"), "utf8"))
    .hooks.SessionStart[0].hooks[0].command).toContain(bin().replaceAll("\\", "/"));
