...
기본 주소는 `http://127.0.0.1:7474`이며 종료는 `Ctrl+C`입니다. 화면은 macOS 데스크톱처럼 배경화면·상단 메뉴바·창·하단 Dock으로 구성되고, 창은 끌어서 옮기거나 크기를 바꾸고 최소화·최대화·닫을 수 있습니다(배치는 브라우저에 저장됩니다). Dock에는 **세션 · 가이드 · 명령 · 설정** 네 앱이 있습니다. 다크 테마가 기본이며 메뉴바의 해·달 버튼이나 설정 창에서 라이트·시스템으로 바꿉니다. 세션 창에서 검색, 제공자·도구·프로젝트 경로 필터, 페이지 이동, 세션 상세를 쓰고, 상세의 **개요 / 기록 이력 / 세션 연결** 탭에서 작업 단서와 연결된 세션을 봅니다. 좁은 화면(760px 이하)에서는 창이 전체 화면으로 바뀌고 Dock이 탭바가 됩니다. CLI가 저장한 변경은 3초 안에 화면에 반영됩니다.
