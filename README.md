...
`latest`와 `--codex/--claude/--grok` 결과에는 **조회 범위**가 표시됩니다. 기본값은 `전체 프로젝트`, `latest --cwd`를 지정하면 정규화된 절대경로입니다. JSON에서는 `scope.cwd`가 `null`이면 전체 프로젝트이고 문자열이면 해당 경로와 정확히 일치하는 프로젝트입니다. 기록이 없을 때도 오류의 `details.scope`에 요청한 범위를 담으며, 다른 프로젝트로 자동 전환하지 않습니다. 특정 프로젝트의 이전 작업을 요청받은 Agent는 `latest <별칭> --cwd <프로젝트 절대경로> --json`을 사용하세요.

화면의 **Agent Session ID**는 원본 Agent가 부여한 실제 세션 ID(`providerSessionId`)이며 CLI 조회와 원본 세션 탐색에 사용합니다. **Relay 내부 ID**(`id`)는 웹 API와 Relay 내부 연결용입니다. CLI에 내부 ID를 잘못 입력하면, 일치하는 내부 기록이 있는 경우 실제 세션 ID·provider·현재 저장소 경로를 포함한 `relay show` 명령을 안내합니다. JSON 오류의 `details.command`와 `details.shell`에서도 같은 안내를 확인할 수 있습니다. 기존 `SESSION_NOT_FOUND` 오류 코드와 종료 코드 3은 유지하며, 실제 Agent Session ID와 일치하는 기록이 있으면 우선 조회합니다. `--provider`를 지정한 경우 다른 제공자의 기록을 대신 안내하지 않습니다.
...
```text
이름                          Provider/Agent         Agent Session ID                      생성              갱신              요약
--------------------------------------------------------------------------------------------------------------------------
...
복사되는 것은 요약 원문이 아니라 **그 기록을 읽는 조회 명령 한 줄**입니다.

```text
이전 세션 맥락은 `relay show '<Session ID>' --provider '<provider>' --data-dir '<저장소>' --json` 명령을 실행해 확인하고, 그 기록을 참고해 다음 작업에 참고 해주세요.
```

CLI Agent의 입력창은 붙여넣기가 조각으로 나뉘어 들어오면 중간의 줄바꿈을 `Enter`로 읽어, 남은 줄을 버린 채 대화를 먼저 보내는 경우가 있습니다. 그래서 복사 텍스트에는 줄바꿈을 만들지 않고, 요약이나 작업명에 줄바꿈이 있어도 한 줄로 접습니다. 받는 Agent는 이 명령을 실행해 마지막 기록 원문을 직접 읽으므로 `relay` 실행 권한이 필요합니다. 저장소 경로를 확인할 수 없을 때는 `--data-dir` 없이 기본 저장소를 보는 명령을 복사합니다.
...
상세 패널은 **개요 / 기록 이력 / 세션 연결** 탭으로 나뉩니다. 전체 요약, 모델·경로·기록 시각·식별자, 순서에 따른 맥락 이력, 이전·후속 세션을 확인할 수 있습니다. 좁은 화면에서는 상세에 집중하고 목록으로 돌아올 때 필터를 유지합니다. **세션 컨텍스트 복사**는 목록과 상세에서 같은 형식으로, [터미널에서 고를 때와 똑같은 한 줄](#터미널에서-골라-복사하기)을 클립보드에 넣습니다. 다른 AI에 붙여 넣어 이전 세션을 이어받는 데 사용합니다. 요약 원문이나 전체 대화, 원본 로그 위치를 함께 실어 보내지 않습니다. 셸 선택에 따라 명령의 인용이 PowerShell·Bash에 맞게 바뀝니다. 명령만 필요하면 **조회 명령 복사**를, ID만 필요하면 개요의 **세션 식별자 → Agent Session ID만 복사**를 사용하세요. 복사는 명령을 실행하지 않습니다.
